...
`course="random_bank"` を追加する。env 初期化時に多数の random path を一度だけ生成する。各 env は `path_id` と `nearest_idx` を持つ。reset 時に `path_id` と start index の両方をランダムに選ぶ。Phase 2 では path slot の再生成は行わない。

### 変更ファイル

#### `src/vehicle_rl/planner/path_bank.py`
...
- `path_bank[path_id]` を `(N, M)` の active path tensor にコピーしない。
- env ごとの path 選択はすべて `path_id` による advanced indexing / gather で行う。
- projection cost は `(N, 2W + 1)` で、`M` に依存しない。
...
```python
from .path_bank import PathBank
from .random_path import random_clothoid_path_bank
```
...
path_bank_size: int = 1024
path_bank_length_m: float = 1000.0
```

状態を追加する。

```python
self.path_bank: PathBank | None
self._path_id = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
self._nearest_idx = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
```

実装内容:

- `_build_path()` は通常の `Path` course 用として残す。
- `course == "random_bank"` 用に `_build_path_bank()` を追加する。
- `__init__` で `course == "random_bank"` の場合は `self.path = None`、`self.path_bank = self._build_path_bank()` とする。
- `_compute_path_state()` は以下のように分岐する。

```python
if self.path_bank is not None:
    plan, lat_err, hdg_err, s_proj, nearest_idx = self.path_bank.project(
        pos_local_xy,
        yaw,
        self._path_id,
        self._nearest_idx,
    ...
    regular Path.project(...)
```

- bank mode の `_reset_idx()` は以下の流れにする。

```python
self._path_id[env_ids_t] = torch.randint(0, self.path_bank.num_paths, (n_reset,), device=self.device)
idx = self.path_bank.sample_start_indices(self._path_id[env_ids_t], end_margin_samples=...)
self._nearest_idx[env_ids_t] = idx
pos_local_xy, yaw, s_reset = self.path_bank.gather_pose(self._path_id[env_ids_t], idx)
```

- `_update_progress()` は `self.path_bank is not None` の場合、bank の metadata を使う。
  - Phase 2 bank path は default では open path とする。
  - YAML で `phase2_bank.is_loop=true` の場合、wrap logic は `self.path_bank.total_length` を使う。
...
- env ごとに異なる `path_id` を持てる。
- reset 時に `path_id` と `nearest_idx` の両方が変わる。
- env 初期化後に path 生成が発生しない。
- reset 時に `(n_reset, M)` の active path copy が発生しない。
- smoke command:
...
```python
self._path_id[env] == slot_id
```

active slot は上書きしてはいけない。上書きすると、走行中の env から見える道路が episode の途中で突然変わる。
...
- `_maybe_regenerate_path_bank()` は以下の条件をすべて満たすときだけ動く。
  - `self.path_bank is not None`
  - regeneration が有効
  - reset counter が interval をまたいだ
- active mask を作る。

```python
active = torch.zeros(self.path_bank.num_paths, dtype=torch.bool, device=self.device)
active[self._path_id] = True
...
- 現在の env の `path_id` と `nearest_idx` は変更しない。
...
- regeneration 有効時も active slot を上書きしない。
- regeneration をまたいでも、走行中の env に不連続な path 変更が起きない。
...
- bank projection の plan 出力 shape が `(N, K)` になる。
...
8. `src/vehicle_rl/planner/path_bank.py` を追加する。
